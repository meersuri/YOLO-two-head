...
import test_license_plate # import test.py to get mAP after each epoch
from models import *
...
def train():
    ...
    yolo_arch = {'yolov3-tiny': [18,19,29,30]}  #architecture dependent new layer numbers for which requires_grad = True
    ...
    if weights.endswith('.pt'):  # pytorch format
        ...
        cutoff = load_darknet_weights(model, weights, yolo_arch[arch])
        param_list = list(model.named_parameters())
        
        #disable gradients for coco trained layers, new layers for license plate are 18,19 and 29,30 in yolov3-tiny
        
        req_grad = yolo_arch[arch]
        for param in param_list:
            if int(param[0].split('.')[1]) in req_grad:
                continue
            else:
                param[1].requires_grad = False
        
        for param in param_list:
            print(param[0], 'requires_grad = ', param[1].requires_grad)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if opt.prebias:
            ...
            if not (opt.notest or (opt.nosave and epoch < 10)) or final_epoch:
                with torch.no_grad():
                    results, maps = test_license_plate.test(cfg,
                                              data,
                                              batch_size=64,
                                              img_size=opt.img_size,
                                              model=model,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--yolo_arch', type=str, default='yolov3-tiny', help='type of yolo model')
    parser.add_argument('--data', type=str, default='data/license_plate.data', help='*.data file path')
